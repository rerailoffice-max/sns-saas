...
import { Clock, AlertCircle, CheckCircle2, Loader2, ExternalLink, Trash2, Save, MessageSquareText, ArrowUp, ArrowDown } from "lucide-react";
...
export function PostDetailContent({
  ...
  const originalThreadPosts = Array.isArray(meta?.thread_posts) ? (meta.thread_posts as string[]) : null;
            ...
            {threadPosts.map((t, i) => (
              <div key={i} className="bg-muted/50 rounded-lg p-3 relative overflow-hidden">
                <div className="absolute top-1.5 right-2 flex items-center gap-0.5">
                  ...
                  <span className="text-[10px] text-muted-foreground ml-1">{i + 1}/{threadPosts.length}</span>
                </div>
                <p className="text-sm whitespace-pre-wrap break-words pr-16">{t}</p>
              </div>
            ))}
          </div>
        ) : (
